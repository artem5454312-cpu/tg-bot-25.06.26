...
async def handle_photo(message: Message, bot: Bot):
    ...
    try:
        ...
        await message.answer("❌ Не удалось отредактировать фото. Попробуй ещё раз.")
...
async def process_free_text(message: Message, text: str,
    ...
    if any(p in text.lower() for p in delete_all_patterns):
    ...
    if any(p in text.lower() for p in delete_done_patterns):
...
async def _handle_create_pdf(message: Message, data: dict):
    ...
    content = await claude_service.generate_pdf_content(project_name, tasks_data, [], memories_data)
